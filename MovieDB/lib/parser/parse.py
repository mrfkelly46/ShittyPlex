...
class PTN():
    ...
    def parse(self, name):
        ...
        if 'resolution' in self.parts:
            if self.parts['resolution'].endswith('p'):
                pass
            elif self.parts['resolution'].endswith('p'):
                pass
            else:
                self.parts['resolution'] += 'p'

        if 'title' not in self.parts or self.parts['title']=='' and 'episode' not in self.parts:
            match = re.search('\((\d{4})\) (.+)\.(mkv|avi|mp4)', name)
            if match:
                self.parts = {
                    'year': match.group(1),
                    'title': match.group(2),
        ...
        if 'title' in self.parts and self.parts['title'].endswith('.'):
            self.parts['title'] = self.parts['title'][:-1]
        return self.parts
